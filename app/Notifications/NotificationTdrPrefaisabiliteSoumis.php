...
class NotificationTdrPrefaisabiliteSoumis extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    protected function getSubject(): string
    {
        return match($this->typeDestinataire) {
            'dgpd_evaluation' => $this->estResoumission
                ? 'Resoumission : TDR de préfaisabilité à évaluer'
                : 'Nouveau TDR de préfaisabilité à évaluer',
            'dpaf_supervision' => $this->estResoumission
                ? 'Supervision : TDR de préfaisabilité resoumis'
                : 'Supervision : Nouveau TDR de préfaisabilité',
            'equipe_organisation' => $this->estResoumission
                ? 'Mise à jour : TDR de préfaisabilité resoumis'
                : 'Information : TDR de préfaisabilité soumis',
            ...
            default => 'TDR de préfaisabilité soumis',
        };
    }
    ...
    protected function getMessage(): string
    {
        return match($this->typeDestinataire) {
            ...
            'soumetteur_confirmation' => $this->estResoumission
                ? 'Votre TDR de préfaisabilité révisé pour le projet "' . $this->projet->titre_projet .
                  '" a été resoumis avec succès. Il sera évalué par la DGPD.'
                : 'Votre TDR de préfaisabilité pour le projet "' . $this->projet->titre_projet .
                  '" a été soumis avec succès. Il sera évalué par la DGPD.',
            default => 'Le TDR de préfaisabilité pour le projet "' . $this->projet->titre_projet . '" a été soumis.',
        };
    }
