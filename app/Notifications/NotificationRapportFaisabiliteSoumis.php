...
class NotificationRapportFaisabiliteSoumis extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    {
        return [
            'type' => 'rapport_faisabilite_soumis',
    ...
    protected function getSubject(): string
    {
        return match($this->typeDestinataire) {
            'dgpd_validation' => $this->estResoumission
                ? 'Resoumission : Rapport de faisabilité à valider'
                : 'Nouveau rapport de faisabilité à valider',
            'dpaf_supervision' => $this->estResoumission
                ? 'Supervision : Rapport de faisabilité resoumis'
                : 'Supervision : Nouveau rapport de faisabilité',
            'equipe_organisation' => $this->estResoumission
                ? 'Mise à jour : Rapport de faisabilité resoumis'
                : 'Information : Rapport de faisabilité soumis',
            ...
            default => 'Rapport de faisabilité soumis',
        };
    }
    ...
    protected function getMessage(): string
    {
        return match($this->typeDestinataire) {
            'dgpd_validation' => $this->estResoumission
                ? 'Le rapport de faisabilité pour le projet "' . $this->projet->titre_projet .
                  '" a été resoumis après révision par ' . $this->soumetteurNomComplet .
                  '. Veuillez procéder à son évaluation.'
                : 'Un nouveau rapport de faisabilité pour le projet "' . $this->projet->titre_projet .
            ...
            'equipe_organisation' => $this->estResoumission
                ? 'Le rapport de faisabilité pour votre projet "' . $this->projet->titre_projet .
                  '" a été resoumis après révision et est en cours d\'évaluation.'
                : 'Le rapport de faisabilité pour votre projet "' . $this->projet->titre_projet .
                  '" a été soumis et est en cours d\'évaluation.',
            'soumetteur_confirmation' => $this->estResoumission
                ? 'Votre rapport de faisabilité révisé pour le projet "' . $this->projet->titre_projet .
                  '" a été resoumis avec succès. Il sera évalué par la DGPD.'
                : 'Votre rapport de faisabilité pour le projet "' . $this->projet->titre_projet .
                  '" a été soumis avec succès. Il sera évalué par la DGPD.',
            default => 'Le rapport de faisabilité pour le projet "' . $this->projet->titre_projet . '" a été soumis.',
        };
    }
    ...
    {
        return match($this->typeDestinataire) {
            'dgpd_validation' => '/projets/' . $this->projet->hashed_id . '/validation-rapport-prefaisabilite',
