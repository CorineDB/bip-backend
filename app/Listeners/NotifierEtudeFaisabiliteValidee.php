...
use App\Notifications\NotificationEtudeFaisabiliteValidee;
use App\Models\User;
use App\Models\Dpaf;
...
class NotifierEtudeFaisabiliteValidee implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(EtudeFaisabiliteValidee $event): void
    {
        ...
        Log::info('Envoi de notifications pour validation d\'étude de faisabilité', [
        ...
        if ($rapport->soumis_par_id) {
            ...
            if ($soumetteur) {
                $soumetteur->notify(
                    new NotificationEtudeFaisabiliteValidee(
                        $rapport,
                        $projet,
                        $evaluation,
                        $validateur,
        ...
        Log::info('Notifications envoyées avec succès pour validation d\'étude de faisabilité', [
            'rapport_id' => $rapport->id,
        ]);
    }
     ...
     */
    public function failed(EtudeFaisabiliteValidee $event, \Throwable $exception): void
    {
        Log::error('Échec de notification EtudeFaisabiliteValidee', [
