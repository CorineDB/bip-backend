...
use App\Notifications\NotificationRapportEvaluationExAnteSoumis;
use App\Models\User;
use App\Models\Dpaf;
use App\Models\Dgpd;
...
class NotifierRapportEvaluationExAnteSoumis implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(RapportEvaluationExAnteSoumis $event): void
    {
        ...
        Log::info("Envoi de notifications pour soumission de rapport d'évaluation ex-ante", [
        ...
        if ($membresDgpd->isNotEmpty()) {
            Notification::send(
                $membresDgpd,
                new NotificationRapportEvaluationExAnteSoumis(
                    $rapport,
                    $projet,
                    $soumetteur,
                    $estResoumission,
        ...
        Log::info("Notifications envoyées avec succès pour soumission de rapport d'évaluation ex-ante", [
            'rapport_id' => $rapport->id,
            'est_resoumission' => $estResoumission,
        ]);
    }
     ...
     */
    public function failed(RapportEvaluationExAnteSoumis $event, \Throwable $exception): void
    {
        Log::error('Échec de notification RapportEvaluationExAnteSoumis', [
