...
use App\Notifications\NotificationTdrFaisabiliteSoumis;
use App\Models\User;
use App\Models\Dpaf;
use App\Models\Dgpd;
...
class NotifierTdrFaisabiliteSoumis implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(TdrFaisabiliteSoumis $event): void
    {
        $tdr = $event->tdr;
        $projet = $event->projet;
        ...
        Log::info('Envoi de notifications pour soumission de TDR de faisabilité', [
            'tdr_id' => $tdr->id,
            'projet_id' => $projet->id,
        ...
        if ($membresDgpd->isNotEmpty()) {
            Notification::send(
                $membresDgpd,
                new NotificationTdrFaisabiliteSoumis(
                    $tdr,
                    $projet,
                    $soumetteur,
                    $estResoumission,
        ...
        Log::info('Notifications envoyées avec succès pour soumission de TDR de faisabilité', [
            'tdr_id' => $tdr->id,
            'est_resoumission' => $estResoumission,
        ]);
    }
     ...
     */
    public function failed(TdrFaisabiliteSoumis $event, \Throwable $exception): void
    {
        Log::error('Échec de notification TdrFaisabiliteSoumis', [
            'tdr_id' => $event->tdr->id,
