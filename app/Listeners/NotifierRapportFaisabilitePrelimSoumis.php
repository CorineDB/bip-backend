...
use App\Notifications\NotificationRapportFaisabilitePrelimSoumis;
use App\Models\User;
use App\Models\Dpaf;
use App\Models\Dgpd;
...
class NotifierRapportFaisabilitePrelimSoumis implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(RapportFaisabilitePrelimSoumis $event): void
    {
        ...
        Log::info('Envoi de notifications pour soumission de rapport de faisabilité préliminaire', [
        ...
        if ($membresDgpd->isNotEmpty()) {
            Notification::send(
                $membresDgpd,
                new NotificationRapportFaisabilitePrelimSoumis(
                    $rapport,
                    $projet,
                    $soumetteur,
                    $estResoumission,
        ...
        Log::info('Notifications envoyées avec succès pour soumission de rapport de faisabilité préliminaire', [
            'rapport_id' => $rapport->id,
            'est_resoumission' => $estResoumission,
        ]);
    }
     ...
     */
    public function failed(RapportFaisabilitePrelimSoumis $event, \Throwable $exception): void
    {
        Log::error('Échec de notification RapportFaisabilitePrelimSoumis', [
