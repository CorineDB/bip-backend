...
use App\Notifications\NotificationTdrFaisabiliteEvalue;
use App\Models\User;
use App\Models\Dpaf;
...
class NotifierTdrFaisabiliteEvalue implements ShouldQueue
{
     ...
     * Handle the event.
     */
    public function handle(TdrFaisabiliteEvalue $event): void
    {
        $tdr = $event->tdr;
        $projet = $event->projet;
        ...
        Log::info('Envoi de notifications pour évaluation de TDR de faisabilité', [
            'tdr_id' => $tdr->id,
            'projet_id' => $projet->id,
        ...
        if ($tdr->rediger_par_id) {
            ...
            if ($redacteur) {
                $redacteur->notify(
                    new NotificationTdrFaisabiliteEvalue(
                        $tdr,
                        $projet,
                        $evaluation,
                        $evaluateur,
        ...
        Log::info('Notifications envoyées avec succès pour évaluation de TDR de faisabilité', [
            'tdr_id' => $tdr->id,
        ]);
    }
     ...
     */
    public function failed(TdrFaisabiliteEvalue $event, \Throwable $exception): void
    {
        Log::error('Échec de notification TdrFaisabiliteEvalue', [
            'tdr_id' => $event->tdr->id,
