...
class CreateOrUpdateChecklistTdrPrefaisabiliteRequest extends FormRequest
{
    private $canevas_redaction_tdr_prefaisabilite = null;
    ...
    {
        $this->canevas_redaction_tdr_prefaisabilite = Document::whereHas('categorie', function ($query) {
    ...
    public function rules(): array
    {
        return [
                ...
                function ($attribute, $value, $fail) {
                    $exists = Document::where('nom', $value)
                        ->whereHas('categorie', function ($query) {
                            $query->where('slug', 'canevas-tdr-prefaisabilite');
                        })->when($this->canevas_redaction_tdr_prefaisabilite, function($query){
                            $query->where("id","<>", $this->canevas_redaction_tdr_prefaisabilite->id);
                        })->exists();
            ...
            'type' => ['required', 'string', Rule::in(['checklist'])],
