...
class CreateOrUpdateCanevasAppreciationTdrPrefaisabiliteRequest extends FormRequest
{
    private $canevas_appreciation_tdr_prefaisabilite = null;
    ...
    private function validateEvaluationConfigsCoherence($validator)
    {
        ...
        $validAppreciations = collect($optionsNotation)->pluck('appreciation')->filter()->toArray();
                ...
                foreach ($appreciationsConcernees as $appreciation) {
                    if (!in_array($appreciation, $validAppreciations)) {
                        $validator->errors()->add(
                            ...
                            "L'appréciation '{$appreciation}' n'est pas définie dans options_notation. " .
