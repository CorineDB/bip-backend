...
class SoumettreEvaluationPertinenceRequest extends FormRequest
{
    ...
    {
        $user = auth()->user();
        return auth()->check() && ($user->hasPermissionTo('effectuer-evaluation-pertinence-idee-projet') && in_array($user->profilable_type, [Dpaf::class, Organisation::class]) && $user->profilable->ministere);
    }
