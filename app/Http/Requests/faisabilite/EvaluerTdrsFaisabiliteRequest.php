...
class EvaluerTdrsFaisabiliteRequest extends FormRequest
{
    ...
    public function prepareForValidation()
    {
        ...
        $tdrId = $this->route('tdrId') ?? null;

        if ($tdrId) {
            $tdrRepository = app()->make(TdrRepositoryInterface::class);
            $tdr = $tdrRepository->find($tdrId);

            // Vérifier si c'est une réévaluation (le TDR a un parent) et qu'il est de type 'faisabilite'
            if ($tdr && $tdr->type === 'faisabilite' && $tdr->parentId) {
                // Récupérer l'évaluation en cours
                $evaluationEnCours = $tdr->evaluationFaisabiliteEnCours();
