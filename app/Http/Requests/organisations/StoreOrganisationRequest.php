...
class StoreOrganisationRequest extends FormRequest
{
    ...
    {
        $user = auth()->user();
        return auth()->check() && (in_array(auth()->user()->type, ['super-admin', 'dgpd']) || (in_array($user->profilable_type, [Dgpd::class]) && ($user->hasPermissionTo('creer-une-organisation') || $user->hasPermissionTo('gerer-les-organisations')) ));
    }
