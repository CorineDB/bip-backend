...
class IdeeProjetRequest extends FormRequest
{
    ...
    {
    $rules = $this->getValidationRulesByAttribut();
        if (!$this->ficheIdee) {
            return;
        }
        ...
        foreach ($this->input('champs', []) as $index => $champInput) {
            ...
            $isRequis = $champ->meta_options['validations_rules']['required'] ?? false;

            if ($modeFinal || $valeurPresente) {
                // Mode final : tous les champs requis doivent être présents
                // Mode brouillon : valider que si une valeur est donnée
                $rules = $this->buildValidationRulesForChamp($champ);
                $validator->addRules([
                    "champs.$index.valeur" => $rules,
                ]);
            }
        }
    }
    ...
    public function rules(): array
    {
        return [
            ...
            "champs" => [
                "array",
                "min:1",
                "max:" . $this->ficheIdee->all_champs->count(),
            ...
            "champs.*.id" => ["distinct", "required", Rule::exists('champs', 'id')->whereIn("id",  $this->ficheIdee->all_champs->pluck("id"))->whereNull('deleted_at')],
            "champs.*." => [],
            "champs.*.valeur" => [],
    ...
    private function getValidationRulesByAttribut(): array
    {
        return [
            ...
            //'responsableId' => ['required', Rule::exists('utilisateurs', 'id')->whereNull('deleted_at')],
            'secteurId' => ['required', Rule::exists('secteurs', 'id')->where("type", 'sous-secteur')->whereNull('deleted_at')],
            //'ministereId' => ['required', Rule::exists('organisations', 'id')->where('type', 'ministere')->whereNull('deleted_at')],
            ...
            'actions_pag' => ['required', 'array', 'min:1'],
            'actions_pag.*' => [
                'required',
                function ($attribute, $value, $fail) {
                    $exists = ComposantProgramme::where('id', $value)
                    ...
                    if (!$exists) {
                        $fail('Action du pag inconnu');
                    }
                }
            ],
            'piliers_pag' => ['required', 'array', 'min:1'],
            'piliers_pag.*' => [
                'required',
                function ($attribute, $value, $fail) {
                    $exists = ComposantProgramme::where('id', $value)
