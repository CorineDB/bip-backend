...
class UpdateIdeeProjetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'champs.titre_projet' => ['required', 'string', 'max:255', Rule::unique('idees_projet', 'titre_projet')->ignore($idIdeeProjet)->whereNull('deleted_at')],
        ];

        /* if ($this->ficheIdee) {
            $defaultRules = $this->getValidationRulesByAttribut($idIdeeProjet);
            $validationRules = $this->ficheIdee->all_champs->mapWithKeys(function ($champ) use ($defaultRules) {
                ...
                if (isset($defaultRules["{$attribut}.*"])) {
                    $validationRules["champs.{$attribut}.*"] = $defaultRules["{$attribut}.*"];
                }
            ...
            if(isset($validationRules["titre_projet"])){
                $rules["champs.titre_projet"] = [...$rules["champs.titre_projet"], ...$validationRules["champs.titre_projet"]];
            }
        } */
    ...
    private function getValidationRulesByAttribut($idee_projet): array
    {
