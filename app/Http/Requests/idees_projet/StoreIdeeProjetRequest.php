...
class StoreIdeeProjetRequest extends FormRequest
{
    ...
    {
        $user = auth()->user();
        return auth()->check() && ($user->hasPermissionTo('creer-une-idee-de-projet') && $user->type === 'responsable-projet' && in_array($user->profilable_type, [Dpaf::class, Organisation::class]) && $user->profilable->ministere);
        //
    }
    ...
    private function getValidationRulesByAttribut(): array
    {
        ...
        $baseRules = [
            ...
            'sources_financement.*' => [
                'required',
                new HashedExists(Financement::class),
                /* function ($attribute, $value, $fail) {
