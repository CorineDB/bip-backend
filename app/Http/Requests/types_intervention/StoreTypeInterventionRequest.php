...
class StoreTypeInterventionRequest extends FormRequest
{
    ...
    {
        $user = auth()->user();
        return auth()->check() && (in_array(auth()->user()->type, ['super-admin', 'dgpd']) || (in_array($user->profilable_type, [Dgpd::class]) && ($user->hasPermissionTo('creer-un-type-intervention') || $user->hasPermissionTo('gerer-les-types-intervention')) ));
    }
