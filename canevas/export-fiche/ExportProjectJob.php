...
class ExportProjectJob implements ShouldQueue
{
    ...
    protected function getProjects()
    {
        switch ($this->action) {
            case 'single':
                $project = ProjectIdea::find($this->options['id']);
                return $project ? [$project] : [];

            case 'batch':
                $ids = $this->options['ids'] ?? [];
                return ProjectIdea::whereIn('id', $ids)->get();
            ...
            case 'by-status':
                ...
                return $status ? ProjectIdea::where('status', $status)->get() : collect();

            case 'by-date':
                $from = $this->options['from'] ?? now()->subMonth();
                $to = $this->options['to'] ?? now();
                return ProjectIdea::whereBetween('created_at', [$from, $to])->get();
     ...
     * Exporter un projet
     */
    protected function exportProject(ProjectIdea $project, ProjectExportService $exportService, $outputDirectory)
    {
