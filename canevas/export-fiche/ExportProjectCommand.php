...
class ExportProjectCommand extends Command
{
    ...
    protected function exportSingle()
    {
        ...
        if (!$projectId) {
            // Mode interactif
            $projects = ProjectIdea::select('id', 'title', 'bip_number')
                ->orderBy('created_at', 'desc')
                ->limit(10)
                ->get();
            
            $choices = $projects->map(function ($p) {
                return "[{$p->id}] {$p->title} (BIP: {$p->bip_number})";
            })->toArray();
        ...
        $project = ProjectIdea::find($projectId);
        ...
        $this->info("Export du projet : {$project->title}");
        $this->newLine();
    ...
    protected function exportBatch()
    {
        ...
        $projects = ProjectIdea::whereIn('id', $ids)->get();
    ...
    protected function exportByStatus()
    {
        ...
        if (!$status) {
            ...
            $statuses = ProjectIdea::distinct()->pluck('status')->filter()->toArray();
        ...
        $projects = ProjectIdea::where('status', $status)->get();
    ...
    protected function exportByDate()
    {
        ...
        $projects = ProjectIdea::whereBetween('created_at', [$fromDate, $toDate])->get();
     ...
     */
    protected function exportProject(ProjectIdea $project, $showOutput = true)
    {
        $this->stats['total']++;
        $format = $this->option('format');
        $withToc = $this->option('with-toc') !== false;
        $language = $this->option('language');
        $template = $this->option('template');
        
        try {
            // Configurer le service d'export
            $this->exportService->setLanguage($language);
            $this->exportService->setWithTableOfContents($withToc);
            
            if ($template) {
                $this->exportService->setTemplate($template);
            }
            ...
            if (in_array($format, ['pdf', 'both'])) {
                ...
                $pdf = $this->exportService->generatePdf($project);
            ...
            if (in_array($format, ['word', 'both'])) {
                ...
                $this->exportService->generateWord($project, $filepath);
     ...
     */
    protected function generateFilename(ProjectIdea $project, $extension)
    {
     ...
     * Logger les exports
     */
    protected function logExport(ProjectIdea $project, array $files, $status, $error = null)
    {
