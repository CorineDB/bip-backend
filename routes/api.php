...
Route::group(['middleware' => ['cors', 'json.response'], 'as' => 'api.'], function () {
    ...
    Route::post('/auth/decrypt', function (Request $request) {
        try {
            ...
            // Retourner le token
            return response()->json(['statut' => 'success', 'message' => 'Authentification réussi', 'data' => new LoginResource(json_decode($token)), 'statutCode' => Response::HTTP_OK], Response::HTTP_OK)/*->withCookie('XSRF-TOKEN', $data['access_token'], 60*3)*/;
